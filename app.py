...
st.set_page_config(
    page_title="IARA: Recursos Aquáticos", layout="wide", page_icon="🌊"
)
st.title("🌊 IARA: Interface de Análise de Recursos Aquáticos")
st.markdown(
    "Plataforma de Monitoramento Costeiro e Análise Estatística de Parâmetros"
    " Oceanográficos."
)
...
REGIOES = {
    "Garopaba - SC (Sul)": {
        "slug": "garopaba",
        "bounds": [-48.75, -48.50, -28.15, -27.95],
    },
    "Baía de Guanabara - RJ (Sudeste)": {
        "slug": "guanabara",
        "bounds": [-43.30, -42.90, -23.00, -22.70],
    },
    "Abrolhos - BA (Nordeste)": {
        "slug": "abrolhos",
        "bounds": [-39.20, -38.60, -18.20, -17.60],
    },
    "Foz do Amazonas - AP/PA (Norte)": {
        "slug": "amazonas",
        "bounds": [-50.50, -49.50, 0.50, 1.50],
    },
}
...
modo_analise = st.sidebar.radio(
    "Modo de Cobertura:", ["📍 Hotspots Estáticos", "🌐 Qualquer Ponto do Brasil"]
)
ano_escolhido = st.sidebar.selectbox(
    "Selecione o Ano de Análise:", ["2025", "2026"]
)
...
if modo_analise == "📍 Hotspots Estáticos":
  regiao_escolhida = st.sidebar.selectbox(
      "Selecione a Região:", list(REGIOES.keys())
  )
  ...
  lat_alvo = st.sidebar.number_input(
      "Latitude (Ex: -23.0 para RJ):",
      min_value=-35.0,
      max_value=5.0,
      value=-23.00,
      step=0.1,
  )
  lon_alvo = st.sidebar.number_input(
      "Longitude (Ex: -43.1 para RJ):",
      min_value=-55.0,
      max_value=-30.0,
      value=-43.10,
      step=0.1,
...
def carregar_e_processar(variavel, ano):
  ...
  if modo_analise == "📍 Hotspots Estáticos":
    ...
    arquivo = baixar_dados_coordenada(
        info["dataset_id"], info["var"], lat_alvo, lon_alvo, ano
    )
...
tab_series, tab_correlacao = st.tabs(
    ["📈 Séries Temporais", "📊 Análise de Correlação"]
)
...
with tab_series:
  variavel_escolhida = st.selectbox(
      "Selecione a Variável para Visualização Diária:", list(DATASETS.keys())
  )

  # Validação das condições especiais
  is_temp_2026 = (
      variavel_escolhida == "Temperatura da Superfície"
      and ano_escolhido == "2026"
  )
  is_amazonas_turbidez = (
      variavel_escolhida == "Turbidez (Sedimentos)"
      and modo_analise == "📍 Hotspots Estáticos"
      and regiao_escolhida == "Foz do Amazonas - AP/PA (Norte)"
  )

  asterisco_red = " :red[*]" if (is_temp_2026 or is_amazonas_turbidez) else ""
  ...
  with col_dados:
    ...
    if ds is not None:
      ...
      if (
          "Temperatura" in variavel_escolhida
          and float(dados_brutos.mean(skipna=True)) > 200
      ):
        dados_brutos = dados_brutos - 273.15

      serie_diaria = dados_brutos.mean(
      ...
      if not df.empty:
        ...
        if modo_analise == "📍 Hotspots Estáticos":
          titulo_diag = (
              f"📊 Diagnóstico: {variavel_escolhida} em {regiao_escolhida}"
              f" ({ano_escolhido}){asterisco_red}"
          )
        else:
          titulo_diag = (
              f"📊 Diagnóstico: {variavel_escolhida}"
              f" ({ano_escolhido}){asterisco_red}"
          )
        ...
        col_m1.metric(
            "Média do Período",
            f"{media:.2f} {DATASETS[variavel_escolhida]['unidade']}",
        )
        col_m2.metric(
            "Pico Máximo",
            f"{maximo:.2f} {DATASETS[variavel_escolhida]['unidade']}",
        )
        col_m3.metric(
            "Valor Mínimo",
            f"{minimo:.2f} {DATASETS[variavel_escolhida]['unidade']}",
        )
        ...
        fig = px.line(
            ...
            labels={
                ...
                var_nome: (
                    "Concentração"
                    f" ({DATASETS[variavel_escolhida]['unidade']})"
                ),
            },
            color_discrete_sequence=[DATASETS[variavel_escolhida]["cor"]],
            markers=True,
        )
        st.plotly_chart(fig, use_container_width=True)

        # Exibição condicional das notas técnicas
        if is_temp_2026:
          st.info(
              "**Nota (*):** Para garantir o máximo rigor acadêmico,"
              " esta plataforma utiliza dados térmicos Reprocessados (REP)."
              " Devido ao processo de calibração e validação manual dos"
              " satélites com boias oceânicas reais pela agência Copernicus, há"
              " uma defasagem natural de processamento de alguns meses,"
              " limitando a série histórica de 2026 até o dia 31 de Março."
          )

        if is_amazonas_turbidez:
          st.info(
              "**Nota (*):** Em águas de estuário hiperturbidas (Águas"
              " de Caso 2), como a pluma do Rio Amazonas, a reflectância óptica"
              " atinge a saturação física. Para mitigar distorções"
              " instrumentais, o algoritmo global de turbidez (SPM) do"
              " Copernicus adota um limite máximo de corte (teto metodológico)"
              " de 100 g/m³."
          )
      ds.close()
    else:
      st.warning(
          "Dados não disponíveis localmente ou aguardando requisição ao"
          " satélite."
      )
...
with tab_correlacao:
  ...
  with col_sel_x:
    var_x = st.selectbox(
        "Variável Independente (Eixo X):", list(DATASETS.keys()), index=1
    )
  with col_sel_y:
    var_y = st.selectbox(
        "Variável Dependente (Eixo Y):", list(DATASETS.keys()), index=0
    )
  ...
  if var_x == var_y:
    ...
    if ds_x is not None and ds_y is not None:
      ...
      df_x = (
          dados_x.mean(dim=["latitude", "longitude"], skipna=True)
          .to_dataframe()
          .reset_index()
      )
      df_x = (
          df_x[["time", var_nome_x]]
          .dropna()
          .rename(columns={var_nome_x: "valor_x"})
      )
      ...
      df_y = (
          dados_y.mean(dim=["latitude", "longitude"], skipna=True)
          .to_dataframe()
          .reset_index()
      )
      df_y = (
          df_y[["time", var_nome_y]]
          .dropna()
          .rename(columns={var_nome_y: "valor_y"})
      )
      ...
      if not df_merged.empty:
        ...
        with col_desc:
          st.markdown(
              "**Grau de Associação:** Coeficiente calculando o acoplamento"
              " ecológico entre as variáveis."
          )
        ...
        slope, intercept = np.polyfit(
            df_merged["valor_x"].values, df_merged["valor_y"].values, 1
        )
        x_trend = np.linspace(
            df_merged["valor_x"].min(), df_merged["valor_x"].max(), 100
        )
